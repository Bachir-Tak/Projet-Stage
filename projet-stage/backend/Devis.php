...
if ($method == 'GET') {
    ...
    } else if (isset($_GET["generatedPrint"])) {
        $sql = 'INSERT INTO historique(action,date,ice) VALUES (:p, :z,:y)';
        $req = $db->prepare($sql);
...
if ($method == 'DELETE') {
    ...
    if (isset($_POST["actif"])) {
        ...
        $req = $db->prepare($sql);
        $req->execute([
            'p' => "La facture du client " . $_POST["client"] . "  dont l'id est " . $_POST["id"] . " a été envoyée à la corbeille
",
            'z' => date("Y-m-d h:i:s"),
            'y' => $_POST["ice"]
        ]);
...
if ($method == 'PUT') {
    ...
    if (isset($_POST["actif"])) {
        ...
        $req = $db->prepare($sql);
        $req->execute([
            'p' => "La facture du client " . $_POST["client"] . " dont l'id est " . $_POST["id"] . " a été renvoyée à la corbeille
",
            'z' => date("Y-m-d h:i:s"),
            'y' => $_POST["ice"]
        ]);
