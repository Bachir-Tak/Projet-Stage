...
function Devis_Edit() {
  ...
  function Sendo(event) {
    ...
    axios
      ...
      .then((data) => {
        Swal.fire("Validé !", "Devis modifiée!", "success");
      });
  }
  ...
  function fetchInvoice() {
    axios
      .get("http://localhost/Projet%20Stage/projet-stage/backend/Devis.php", {
        params: { ice: window.userICE, id_modif: id, devis: true },
      })
      .then((data) => {
        var tab = [];
        var inv = data.data["inv"][0];
        var prodInv = data.data["produits"];
        document.getElementById("date_facture").value = inv["date_facture"];
        setrowtab((oldArray) => [
          ...oldArray,
          { nom_client: inv["nom_client"] },
        ]);
        document.getElementById("TotalTVA").value = inv["TotalTVA"];
        document.getElementById("TotalHT").value = inv["TotalHT"];
        document.getElementById("TotalTTC").value = inv["TotalTTC"];
        settrueValue(Number((inv["TotalTTC"] * 100) / (100 - inv["Remise"])));
        prodInv.map((d) => {
          tab.push({
            1: d["nom_produit"],
            2: d["quantité"],
  ...
  useEffect(() => {
    if (rowtab.length == 0) {
      ...
      document.getElementById("client").value =
        rowtab[rowtab.length - 1]["nom_client"];
    }
  }, [rowtab]);
  useEffect(() => {
    var totalTTC = document.getElementById("TotalTTC");
                    ...
                    {listProds.map((e) => (
                      <tr>
                        ...
                        <td onBlur={(event) => change1(e, event)}>
                          {" "}
                          <input type="number" defaultValue={e[3]} />
                        </td>
